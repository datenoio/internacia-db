...
session.headers.update({"User-Agent": "Internacia-DB Link Validator/1.0"})
...
def get_wikidata_entity_info(qid: str) -> tuple[bool, dict | None]:
    """
    ...
    try:
        ...
        # Surface API failures instead of silently treating them as
        # missing entities, so transient outages are distinguishable.
        typer.echo(f"WARN: Wikidata API error for {qid}: {e}", err=True)
        return False, None
            ...
            if not re.match(r"^Q\d+$", wikidata_id):
                ...
                if wikidata_ids_in_links:
                    if wikidata_id not in wikidata_ids_in_links:
                        errors.append(
                            f"wikidata_id ({wikidata_id}) doesn't match any wikidata link Q-numbers: {wikidata_ids_in_links}"
                        )
...
def validate(
    directory: Path = typer.Argument(
        default=None, help="Directory containing intblocks YAML files (default: data/intblocks under the repo root)"
    ),
    check_http: bool = typer.Option(True, "--check-http/--no-check-http", help="Check if URLs are accessible via HTTP"),
    check_wikidata: bool = typer.Option(
        True, "--check-wikidata/--no-check-wikidata", help="Validate Wikidata entities match organization names"
    ),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show detailed output for each file"),
    max_errors: int = typer.Option(100, "--max-errors", help="Maximum number of errors to display"),
):
    """Validate all links and wikidata_id values in intblocks YAML files."""
    if directory is None:
        directory = Path(__file__).resolve().parents[1] / "data" / "intblocks"
