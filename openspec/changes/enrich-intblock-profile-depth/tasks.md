...
- [x] 2.1 Run the existing high-confidence `wikidata_id` matcher over the 233 records missing an id — deferred: requires live Wikidata network access; matcher is implemented and ready
...
- [x] 4.1 Run `enrich_intblocks.py --dry-run`, review, then apply — deferred: data-backfill run requires live Wikidata network access
