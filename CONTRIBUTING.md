...
Thank you for contributing. This repository is **data-as-code**: the YAML files under `data/countries/` and `data/intblocks/` are the authoritative sources, and everything in `data/datasets/` (except `blocktypes.yaml`) is generated by the build.
...
- Every `blocktype` value must exist in `data/datasets/blocktypes.yaml`.
